...
task follow_line()
{
	...
	while(1){
		...
		motor[motorB] = (3*(light_right-light_left*0.5-20));
		motor[motorC] = (3*(light_left-light_right*0.5-20));
		...
		if((light_right < 40) && (light_left < 40)){
			...
			// Loop tot input.
			while(1){
				char * direction = ask_direction();
				if(direction == "LEFT") {
					// Loop tot de lichtwaarde van sensor 1 hoger is dan 60.
					while(SensorValue[S1] < 60){
						wait1Msec(10);
					}
					break;
				} else if(direction == "RIGHT") {
					// Loop tot de lichtwaarde van sensor 2 hoger is dan 60.
					while(SensorValue[S2] < 60){
						wait1Msec(10);
					}
					break;
				} else {
